...
- TEST: checkout slice rewritten (12 backend tests: gates, fee math, session reuse, non-complete session → failed, refund, cross-tenant); mock redirect flow driven end-to-end in a real browser (dialog → return page → confirm → Pago → Avaliar oficina, zero console errors); real Stripe verified up to the hosted checkout page (`cs_test_...` sessions, `m.stripe.com` submit works outside headless automation) — card completion is a manual step (Stripe hCaptcha blocks headless bots)
- FEAT: Payment processing (Phase 6) — new tenant-scoped `payments` table (integer cents for amount/10% platform fee/workshop share, unique `service_order_id`, `status` pending→succeeded→refunded/failed, `stripe_payment_intent_id`) + `workshop_ratings.service_order_id` link (migration `894ad4d4a168`, verified up/down/up)
- FEAT: `/payments` API — `POST /service-orders/{id}/intent` (CLIENT, completed order, amount derived server-side from `final_cost`, pending-intent reuse, `MockProvider` when no `STRIPE_SECRET_KEY`/`PAYMENT_PROVIDER=mock`), `POST /{id}/confirm` (synchronous provider verification, idempotent, order `completed → paid` + workshop notification/WS), `GET /service-orders/{id}` (role-aware), `POST /{id}/refund` (WORKSHOP, full amount, `paid → refunded` + client notification/WS); provider abstraction in `src/utils/payments.py` (protocol + Mock/Stripe, no SDK imports outside it)
- FEAT: Lifecycle gains terminal `paid`/`refunded` statuses (matrix: `completed → paid` CLIENT via payment, `paid → refunded` WORKSHOP); surfaced in client `STATUS_META` ("Pago"/"Reembolsado"), workshop status colors, both dashboards
- FEAT: Order-anchored reviews — `POST /workshop-ratings` accepts `service_order_id` (exactly one anchor, client-owned paid order, one review per paid order, PT-BR 400s); client services page shows "Avaliar oficina" after payment reusing `RatingModal`
- FEAT: Frontend payments — `payment-service` adapter, `PaymentDialog` (Stripe Elements CardElement when `VITE_STRIPE_PUBLISHABLE_KEY` set, "Simular pagamento" mock fallback), "Pagar" on completed client orders, `RefundPaymentButton` + payment-state captions in both workshop order dialogs
- DEV: `stripe` SDK (backend), `@stripe/stripe-js` + `@stripe/react-stripe-js` (web); `.env.example` documents `STRIPE_SECRET_KEY`/`STRIPE_PUBLISHABLE_KEY`/`PAYMENT_PROVIDER`
- TEST: 27 new backend tests (payment lifecycle, fee math 100.50 → 10050/1005/9045 cents, cross-tenant denial, idempotent confirm, failed-provider state, refund gating, paid-gated reviews, WS pushes; full suite 138 green); Vitest slices for the payment adapter, payment-mode, `STATUS_META.paid/refunded`, and the order review payload (32 green); tsc + vite build green
...
- TEST: backend slices for reject lifecycle (notify workshop, terminal state, required cost/date), parts persistence + cost derivation (final_cost = parts + labor, estimated-cost fallback, negative unit rejected), breakdown access per role + cross-tenant denial, and the reject WS push (31 targeted tests, full suite 108 green); Vitest slice for the service adapters (reject/breakdown/parts payload) + `STATUS_META.rejected`
- FIX: create-order error when the client has no vehicle in the workshop's tenant is now PT-BR and actionable ("O cliente X não possui veículo cadastrado na sua oficina...") instead of "No vehicle found for user id 5"
