...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: int):
        ...
        logger.info(
            f"[WS] User {user_id} connected. Online: {list(self.active_connections.keys())}"
        )

    def disconnect(self, user_id: int):
        self.active_connections.pop(user_id, None)
        logger.info(
            f"[WS] User {user_id} disconnected. Online: {list(self.active_connections.keys())}"
        )
    ...
    async def send_to_user(self, user_id: int, message: dict) -> bool:
        ...
        if not websocket:
            logger.warning(
                f"[WS] Cannot deliver to user {user_id}: not connected. Online: {list(self.active_connections.keys())}"
            )
            return False
        try:
