...
def repo_create_vehicle(db: Session, tenant_id: UUID | str, user_id: int, vehicle_data: dict) -> Vehicle:
...
def repo_get_vehicle_by_email(db: Session, email: str, tenant_id: UUID | str) -> list[Vehicle]:
    return (
        db.query(Vehicle)
        .join(User)
