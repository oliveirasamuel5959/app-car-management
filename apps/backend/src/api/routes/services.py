...
from src.schemas.services import ServiceCreate, ServiceRead, ServiceUpdate
...
def create_service(
    ...
    try:
        ...
        return service.create_service(
            service_in, user_id=int(user_id), tenant_id=tenant_id
        )
...
def get_my_services(
    ...
    try:
        ...
        scoped_tenant_id = None if role == "CLIENT" else tenant_id
        result = service.get_services_by_user_id(
            int(user_id), scoped_tenant_id, user_email=user_email
        )
        return result
    except Exception:
        raise HTTPException(
...
def get_service(
    ...
    try:
        ...
        role = current_user.get("role")
        if role == "CLIENT":
            result = service.get_service_by_client_access(
                service_id,
            ...
            result = service.get_service_by_id(service_id, tenant_id)
        if result is None:
            raise HTTPException(
...
def create_service(
    ...
    try:
        user_id = current_user.get("user_id")
        tenant_id = current_user.get("tenant_id")
        return service.create_service(service_in, user_id=int(user_id), tenant_id=tenant_id)
    except ValueError as e:
        raise HTTPException(
...
def get_my_services(
    ...
    try:
        ...
        role = current_user.get("role")
        scoped_tenant_id = None if role == "CLIENT" else tenant_id
        result = service.get_services_by_user_id(int(user_id), scoped_tenant_id, user_email=user_email)
        return result
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
