...
from src.repositories.vehicle import check_duplicate_plate, repo_create_vehicle, repo_get_vehicle_by_email
...
class VehicleService:
  ...
  def create_vehicle(self, vehicle_in: VehicleCreate, user_id, tenant_id):
    ...
    try:
      return repo_create_vehicle(self.db, tenant_id=tenant_id, user_id=user.id, vehicle_data=vehicle_in.dict(exclude={"user"}))
