...
from src.repositories.workshop_client import (
    repo_check_duplicate_plate_in_workshop, repo_create_workshop_client,
    repo_delete_workshop_client, repo_get_workshop_client_by_id,
    repo_get_workshop_clients_by_workshop_id, repo_update_workshop_client)
...
class WorkshopClientService:
    ...
    def _get_workshop_for_user(self, user_id: int, tenant_id) -> Workshop:
        ...
        if not workshop:
            logger.error(
                f"No workshop found for user_id {user_id} and tenant_id {tenant_id}"
            )
            raise ValueError(
                "No workshop found for the authenticated tenant. Re-authenticate and ensure you are logged into the workshop account for this tenant."
            )
        return workshop

    def _resolve_registered_client_user(
        self, email: str | None, tenant_id
    ) -> User | None:
        if not email:
            return None
    ...
    def _backfill_client_user_links(
        self, clients: List[WorkshopClient], tenant_id
    ) -> None:
        updated = False
        for client in clients:
            ...
            resolved_user = self._resolve_registered_client_user(
                client.email, tenant_id
            )
    ...
    def create_client(
        self, client_in: WorkshopClientCreate, user_id: int, tenant_id
    ) -> WorkshopClient:
        logger.info(
        ...
        if repo_check_duplicate_plate_in_workshop(
            self.db, tenant_id, workshop.id, client_in.vehicle_plate
        ):
            logger.warning(
            ...
            raise ValueError(
                f"A client with plate '{client_in.vehicle_plate}' already exists in this workshop"
            )

        return repo_create_workshop_client(
            self.db, tenant_id, workshop.id, user_id_for_client, client_in.model_dump()
        )

    def get_clients(self, user_id: int, tenant_id) -> List[WorkshopClient]:
        workshop = self._get_workshop_for_user(user_id, tenant_id)
        clients = repo_get_workshop_clients_by_workshop_id(
            self.db, workshop.id, tenant_id
        )
        self._backfill_client_user_links(clients, tenant_id)
        return clients

    def get_client_by_id(
        self, client_id: int, user_id: int, tenant_id
    ...
    def update_client(
        self, client_id: int, update_in: WorkshopClientUpdate, user_id: int, tenant_id
    ) -> Optional[WorkshopClient]:
        workshop = self._get_workshop_for_user(user_id, tenant_id)
        client = repo_get_workshop_client_by_id(self.db, client_id, tenant_id)
        if not client or client.workshop_id != workshop.id:
        ...
        if "vehicle_plate" in update_data:
            if repo_check_duplicate_plate_in_workshop(
                ...
                tenant_id,
                workshop.id,
                update_data["vehicle_plate"],
                exclude_id=client_id,
            ):
                raise ValueError(
                    f"A client with plate '{update_data['vehicle_plate']}' already exists in this workshop"
                )
